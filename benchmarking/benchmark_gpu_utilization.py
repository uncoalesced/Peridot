    ...
    def stop(self):
        ...
        result = {
            ...
            "utilization": {
                ...
                "median": (
                    statistics.median(self.utilizations) if self.utilizations else 0
                ),
                "stdev": (
                    statistics.stdev(self.utilizations)
                    if len(self.utilizations) > 1
                    else 0
                ),
    ...
    while time.time() - start_time < duration_s:
        ...
        try:
            ...
            response = requests.post(
                AI_SERVER_URL, json=payload, headers=headers, timeout=30
            )
...
def main():
    ...
    try:
        ...
        logger.error(
            "Peridot is not running! Please start Peridot Neural Engine first."
        )
        sys.exit(1)
    ...
    logger.info(
        f"  Range: {active_stats['utilization']['min']:.1f}% - {active_stats['utilization']['max']:.1f}%"
    )
