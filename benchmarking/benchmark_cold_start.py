...
def measure_cold_start():
    ...
    try:
        ...
        logger.error(
            "  Peridot is still responding! Cannot accurately measure cold start."
        )
        return None
    except RuntimeError:
    ...
    if not wait_for_health(timeout=200):
        logger.error(
            "  Peridot did not become ready within 200 seconds. Killing process."
        )
        proc.kill()
        return None
    ...
    logger.info(
        f"  Neural Engine is ready! Startup time: {format_duration(startup_time)}"
    )
...
def main():
    ...
    if startup_times:
        ...
        if first_query_times:
            result.add_metadata(
                "avg_first_query_time_s", statistics.mean(first_query_times)
            )
        if total_times:
