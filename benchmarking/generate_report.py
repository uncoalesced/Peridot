...
def calculate_peridot_rating():
    ...
    if hardware_info:
        ...
        if vram >= 8:
            ...
            reasons.append(
                f"[WARN] {vram}GB VRAM detected (Suboptimal, heavy quantization required)."
            )
    ...
    if vram_result:
        ...
        if median_handoff < 600:
            ...
            score += 2
            reasons.append(
                f"[PASS] Handoff latency is acceptable ({median_handoff:.2f}ms)."
            )
        else:
    ...
    if med_result:
        ...
        if tps > 5000:
            score += 3
            reasons.append(
                f"[PASS] Aether-Route semantic cache is highly active ({tps:.2f} t/s)."
            )
        elif tps > 30:
            score += 2
    ...
    if mem_result:
        ...
        if abs(growth) < 50:
            score += 2
            reasons.append(
                f"[PASS] Zero memory leaks detected (Growth: {growth:+.2f}MB)."
            )
        else:
...
def generate_readme_section():
    ...
    if vram_result:
        ...
        output.append(
            f"| FAH pause command | {metadata.get('avg_pause_latency_ms', 0):.2f}ms |"
        )
        ...
        output.append(
            f"**Inference performance: {metadata.get('avg_inference_throughput', 0):.2f} t/s** (unchanged)"
        )
    ...
    if memory_result:
        ...
        output.append(
            f"- **Memory growth: {abs(growth):.0f}MB** (bounded memory [PASS])"
        )
...
def generate_full_benchmarks_doc():
    ...
    for bench_id, bench_title in benchmarks:
        ...
        if bench_id.startswith("inference"):
            ...
            output.append("")
            output.append(
                f"- Average tokens generated: {metadata.get('avg_tokens_generated', 0):.0f}"
            )
            output.append(
                f"- Average elapsed time: {format_duration(metadata.get('avg_elapsed_time', 0))}"
            )
            output.append("")
            ...
            output.append("|--------|-------|")
            output.append(
                f"| Pause command latency | {metadata.get('avg_pause_latency_ms', 0):.2f}ms |"
            )
            output.append(f"| VRAM release time | {stats.get('median', 0):.2f}ms |")
            output.append(
                f"| VRAM freed | {metadata.get('avg_vram_freed_mb', 0):.0f}MB |"
            )
            output.append(
                f"| Inference throughput | {metadata.get('avg_inference_throughput', 0):.2f} t/s |"
            )
            output.append("")
            ...
            output.append(
                f"- Successful queries: {metadata.get('successful_queries', 0)}"
            )
            output.append(
                f"- Initial memory: {metadata.get('initial_memory_mb', 0):.2f}MB"
            )
            output.append(f"- Final memory: {metadata.get('final_memory_mb', 0):.2f}MB")
            output.append(
                f"- Memory growth: {metadata.get('memory_growth_mb', 0):+.2f}MB"
            )
            output.append("")
            ...
            if metadata.get("results_by_size"):
                ...
                baseline = (
                    results_by_size[0]["avg_throughput"] if results_by_size else 0
                )

                for res in results_by_size:
                    degradation = (
                        ((baseline - res["avg_throughput"]) / baseline * 100)
                        if baseline
                        else 0
                    )
                    output.append(
