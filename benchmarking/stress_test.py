        ...
        with open(env_path, "r", encoding="utf-8") as f:
            for line in f:
                if line.startswith("API_KEY="):
                    ...
                    try: SERVER_PORT = int(line.strip().split("=", 1)[1])
                    except: pass
...
def run_health_check() -> bool:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[CRITICAL] Engine responded with anomalies. Status Code: {r.status_code}")
            return False
    except requests.exceptions.RequestException as e:
        print(f"[FATAL] Connection to Kernel failed entirely. Is server.py active? Details: {e}")
        return False
...
def stress_session_persistence():
    ...
    for i, q in enumerate(queries):
        ...
        start = time.time()
        try:
            r = requests.post(f"{SERVER_URL}/ask", json=payload, headers=HEADERS, timeout=60)
            elapsed = time.time() - start
            
            if r.status_code == 200:
                res_data = r.json()
                session_id = res_data.get("session_id")
                print(f" └── [OK] Nominal turn clearance. Session ID: {session_id} | Latency: {elapsed:.2f}s")
            else:
                print(f" └── [ERROR] Route faulted during session processing. Code: {r.status_code}")
                break
...
def hardware_concurrency_load():
    ...
    def hammer_endpoint(thread_id):
        payload = {"query": f"Thread test signature {thread_id}", "prompt": "Ping status signature."}
        try:
            print(f" ├── Thread-{thread_id} launching execution payload...")
            r = requests.post(f"{SERVER_URL}/ask", json=payload, headers=HEADERS, timeout=30)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loop", action="store_true", help="Run the stress test in an infinite loop (e.g. for 24h stability testing)")
        ...
        try:
            while True:
                ...
                if run_health_check():
                    ...
                    print(f"[FATAL] Health check failed on cycle {cycle}. Aborting loop.")
                    sys.exit(1)
