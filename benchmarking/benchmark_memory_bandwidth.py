...
def measure_pcie_bandwidth():
    ...
    if not torch.cuda.is_available():
        print(
            f"{RED}[FATAL] CUDA is unavailable. Hardware abstraction layer failure.{RESET}"
        )
        return None
...
def measure_nvme_bandwidth():
    ...
    print(f"  - Latency: {duration:.4f}s")
    print(
        f"  - Throughput: {YELLOW}{bandwidth_mb_s:.2f} MB/s ({bandwidth_gb_s:.2f} GB/s){RESET}"
    )
...
def main():
    ...
    if all([vram_bw, pcie_bw, nvme_bw]):
        ...
        print(
            f"{RED}[FATAL] Telemetry collection failed. Could not compile threshold matrix.{RESET}"
        )
